-- Verify exact session metadata structure for display names
...
-- 6. Check Google OAuth vs regular signup metadata differences
SELECT 
    CASE 
    ...
    END as signup_type,
    COUNT(*) as total,
    COUNT(CASE WHEN raw_user_meta_data->>'display_name' IS NOT NULL THEN 1 END) as has_display_name,
    COUNT(CASE WHEN raw_user_meta_data->>'full_name' IS NOT NULL THEN 1 END) as has_full_name,
    array_agg(DISTINCT jsonb_object_keys(raw_user_meta_data)) as metadata_keys
FROM auth.users 
WHERE raw_user_meta_data IS NOT NULL
GROUP BY 1;
