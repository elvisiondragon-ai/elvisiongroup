...
BEGIN
  ...
  IF existing_sub.id IS NOT NULL THEN
    ...
    SET 
        subscription_end_date = pro_subscriptions.subscription_end_date + (days_to_add || ' days')::INTERVAL,
        subscription_type = waiting_record.subscription_type,
        amount_paid = pro_subscriptions.amount_paid + waiting_record.amount_paid,
