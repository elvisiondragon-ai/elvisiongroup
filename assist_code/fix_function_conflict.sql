-- This script resolves the function overloading conflict by removing all old versions
-- of the award_xp and award_audio_xp functions and creating the single, correct version.

-- Step 1: Drop all potentially conflicting functions.
-- This drops the function with the 'minutes_listened' parameter that was causing the conflict.
DROP FUNCTION IF EXISTS public.award_audio_xp(uuid, integer, boolean);

-- This drops the main function so it can be recreated cleanly.
DROP FUNCTION IF EXISTS public.award_xp(uuid, integer, text, jsonb, text);


-- Step 2: Re-create the correct award_xp function.
-- This is the definitive version that increments total_verses before the XP check.
CREATE OR REPLACE FUNCTION public.award_xp(p_user_id uuid, p_xp_amount integer, p_activity_type text, p_metadata jsonb DEFAULT NULL::jsonb, p_reason text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    -- Step 1: Unconditionally increment total_verses for verse or audio completions.
    IF p_activity_type IN ('verse_completion', 'audio_completion') THEN
        UPDATE public.profiles
        SET total_verses = COALESCE(total_verses, 0) + 1
        WHERE user_id = p_user_id;
    END IF;

    -- Step 2: Check daily XP limit.
    ...
    -- If daily limit already reached, return early, but after the verse count has been incremented.
    ...
    -- Note: total_verses is no longer incremented here.
    UPDATE public.profiles
