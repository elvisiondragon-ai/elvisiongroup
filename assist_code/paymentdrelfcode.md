...
import Adress from '@/components/adress'; // Assuming this component provides address inputs
...
// --- Start of extracted payment-related code from src/pages/drelf.tsx ---
...
export default function DrelfPaymentPage() {
  ...
  const { user } = useAuth();
  const { proStatus } = usePro(); // Assuming usePro is related to subscription status affecting payment options

  const productId = 'drelf_collagen_1x_600k';
  const productName = 'Drelf Collagen';
  const price = 600000;

  // State for product quantity and user details
  ...
  // State for payment method and process
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('QRIS');
  const [loading, setLoading] = useState(false);
  const [paymentData, setPaymentData] = useState<any>(null); // Stores data received after creating payment
  const [showPaymentInstructions, setShowPaymentInstructions] = useState(false); // Controls display of payment instructions

  // Effect to populate user details if logged in
  useEffect(() => {
    if (user) {
  ...
  // Effect to restrict payment methods based on total amount
  useEffect(() => {
  ...
  const handleCreatePayment = async () => {
    ...
    setLoading(true);
    try {
      // Invoke Supabase Edge Function to create payment via Tripay
      ...
      // Handle errors from Supabase function
      if (error || !data?.success) {
        if (selectedPaymentMethod === 'BCA_MANUAL') {
          // Special handling for manual BCA transfer if the function fails
          setPaymentData({
            paymentMethod: selectedPaymentMethod,
            amount: totalAmount,
            status: 'UNPAID',
      ...
      if (data?.success) {
        setPaymentData(data); // Store payment data returned by the function
        setShowPaymentInstructions(true); // Show payment instructions UI
        toast({
  ...
  // Effect to listen for payment status updates via Supabase Realtime
  useEffect(() => {
    if (!showPaymentInstructions || !paymentData?.tripay_reference) return; // Only listen if instructions are shown and reference exists
    
    const tableName = 'global_product'; // Table where payment status is updated
    const channel = supabase
      .channel(`payment-status-drelf-${paymentData.tripay_reference}`) // Unique channel for this payment
        ...
        (payload) => {
          ...
          if (payload.new?.status === 'PAID') {
            toast({
                ...
                action: (() => {
                      // Generate WhatsApp message with payment details for confirmation
      ...
      ).subscribe(); // Start listening to the channel
  ...
  // Conditional rendering: Show payment instructions if a payment has been initiated
...
`Mohon konfirmasi pesanan saya. Terima kasih.`)}`}
  ...
  // Initial payment form rendering
  return (
    <div className="min-h-screen bg-background pb-32">
      <Toaster />
      {/* WhatsAppButton and Logout/Login omitted for brevity but are present in original file */}
      <div className="p-6 pb-4">
        <div className="flex items-center justify-between mb-6">
            ...
            {user ? (
                <Button variant="outline" onClick={() => {/* handleLogout function omitted for brevity */}}>Logout</Button>
            ) : (
                <Button variant="outline" onClick={() => navigate('/auth?redirect=/drelf')}>Login</Button>
            )}
        </div>
      </div>
