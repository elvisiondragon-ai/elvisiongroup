-- FIX EMAIL AMOUNT: Show new payment amount, not total

-- Option 1: Store new payment amount in waiting_payment table
-- Add column to track individual payment amounts
ALTER TABLE public.waiting_payment 
ADD COLUMN IF NOT EXISTS individual_payment_amount NUMERIC DEFAULT 0;

-- Option 2: Modify activate_pro_subscription to return payment details
CREATE OR REPLACE FUNCTION public.activate_pro_subscription(
    p_tripay_reference text, 
    p_payment_method text DEFAULT NULL
) RETURNS jsonb  -- Changed return type to include payment info
...
BEGIN
  ...
  -- Return subscription ID with payment details for email
  RETURN jsonb_build_object(
    'subscription_id', new_subscription_id,
    'payment_amount', waiting_record.amount_paid,  -- Individual payment amount for email
