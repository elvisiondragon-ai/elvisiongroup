-- Fix award_xp function - COMPLETE and SIMPLE
-- This fixes the broken INSERT that caused XP not to work after analytics columns were added
...
DECLARE
  ...
  result jsonb;
BEGIN
  -- Check daily XP limit first (30 XP max per day)
  ...
  -- If no profile exists, create one with ONLY essential columns
  IF NOT FOUND THEN
    INSERT INTO public.profiles (
      ...
      (SELECT COALESCE(raw_user_meta_data->>'display_name', email) FROM auth.users WHERE id = p_user_id),
      0,
      0,
      0,
      0,
  ...
  -- Update activity counters instantly based on activity type
  IF p_activity_type = 'verse_completion' THEN
    UPDATE public.profiles
    SET total_verses = total_verses + 1
    WHERE user_id = p_user_id;
  ELSIF p_activity_type = 'journal_completion' THEN
    UPDATE public.profiles
    SET total_journal = total_journal + 1
    WHERE user_id = p_user_id;
  ELSIF p_activity_type = 'elite_habit_completion' THEN
    UPDATE public.profiles
    SET total_elite_habit = total_elite_habit + 1
    WHERE user_id = p_user_id;
  END IF;
  ...
  IF new_level >= 3 AND old_level < 3 THEN
    ...
    SET achievements = CASE
      ...
      ELSE array_append(COALESCE(achievements, '{}'), 'level_3')
    END
    WHERE user_id = p_user_id;
