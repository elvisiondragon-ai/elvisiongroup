-- ===========================================
-- FIX CLEAN PRO BADGES FROM ACHIEVEMENTS COLUMN
-- ===========================================
...
-- 2. Remove pro-related achievements using proper array filtering
UPDATE profiles
SET achievements = (
    SELECT ARRAY(
        SELECT element
        FROM unnest(achievements) AS element
        WHERE element NOT ILIKE '%pro%'
        AND element NOT ILIKE '%badge%'
    )
)
