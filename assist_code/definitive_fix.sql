-- This script creates a new, uniquely named function to avoid conflicts.
-- It correctly increments total_verses before checking the XP limit.

-- Step 1: Drop the old conflicting functions to be safe.
...
-- Step 2: Create the new, correctly implemented function with a unique name.
CREATE OR REPLACE FUNCTION public.grant_xp_and_increment_verse(p_user_id uuid, p_xp_amount integer, p_activity_type text, p_metadata jsonb DEFAULT NULL::jsonb, p_reason text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
