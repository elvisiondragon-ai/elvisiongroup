...
async function addToMailketingList(email, name) {
  try {
    console.log(`📋 Adding ${email} to Mailketing list ${LIST_ID} for Ebook eL-Vision...`);
...
const handler = async (req)=>{
  console.log('🚀 Mailketing Ebook eL-Vision Email Function Started');
  ...
  try {
    ...
    const subscriptionType = body.subscriptionType || 'Ebook eL-Vision'; // Default to Ebook eL-Vision
    ...
    const safeSubscriptionType = subscriptionType || 'Ebook eL-Vision';
    await addToMailketingList(recipientEmail, userName);
    const subject = "🎉 Pembayaran Berhasil! Link Ebook eL-Vision Anda";
    const htmlContent = `<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>🎉 Pembayaran Berhasil! Ebook eL-Vision Anda Siap</title>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <h1 class="main-title">🎉 PEMBAYARAN BERHASIL! Ebook eL-Vision Anda Siap</h1>
            <p class="subtitle">Terima kasih telah membeli Ebook eL-Vision!</p>
        </div>
        ...
        <div class="content">
            ...
            <p class="description">
                <strong>Terima kasih!</strong> Pembayaran Anda untuk Ebook eL-Vision telah berhasil diproses. 
                Silakan unduh Ebook eL-Vision Anda melalui tautan di bawah ini:
            </p>

            <div class="cta-section">
                <a href="https://drive.google.com/drive/folders/1ZQ4LsWFnuuRJTNu1vfZc5-hJDfOGzJsU?usp=sharing" class="cta-button">
    ...
    console.log("✅ Mailketing Ebook eL-Vision email sent successfully");
    try {
      await sendMailketingEmail('support@elvisiongroup.com', `SENT: ${subject}`, htmlContent, 'Support Team');
      console.log("✅ BCC copy sent to support@elvisiongroup.com");
    } catch (bccError) {
      console.error("⚠️ Failed to send BCC copy:", bccError);
    }
    return new Response(JSON.stringify({
      success: true,
      message: 'Ebook eL-Vision email sent successfully via Mailketing',
    ...
    console.error("❌ Error sending Mailketing Ebook eL-Vision email:", error);
    return new Response(JSON.stringify({
      success: false,
      error: error.message,
      details: error.stack
