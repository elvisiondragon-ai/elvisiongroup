...
function generateCallbackSignature(jsonString: string, privateKey: string): string {
...
serve(async (req) => {
  ...
  try {
    ...
    if (status === 'PAID') {
      ...
      const { data: waitingData, error: selectError } = await supabase
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          product_name: 'Drelf Collagen', // Or waitingData.subscription_type
          status: 'PAID',
          amount: total_amount,
          // Address is missing from waiting_payment, needs to be added there first
        });
