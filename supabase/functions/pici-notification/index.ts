...
serve(async (req) => {
  console.log("🚀 PICI-NOTIFICATION Triggered");
  ...
  try {
    ...
    // Supabase webhook wraps the new record in "record"
    ...
    const { nama, whatsapp, email, status, company, bidang_usaha, domisili, tujuan_meetup } = newMember;
    ...
    // 1. Pesan ke Member
    const memberMessage = `Halo ${nama}, salam kenal.\n\nSaya El Reyzandra.\n\nTerima kasih telah mendaftarkan diri di *PICI (Private Inner Circle Insider)*.\n\nInformasi Anda telah saya terima:\nNama: ${nama}\nStatus: ${status}\nCompany: ${company}\nDomisili: ${domisili}\n\nPICI adalah ruang bagi founder untuk menjernihkan pikiran dan berbagi energi. Jika profil Anda cocok dengan visi lingkaran kecil ini, saya atau tim akan segera menghubungi Anda kembali untuk mengatur jadwal pertemuan (meet-up) di Jakarta Barat.\n\nSampai jumpa.\n\nSalam,\nEl Reyzandra\neL Vision Group`;
    ...
    const adminMessage = `🔔 *PICI MEMBER BARU*\n\nAda pendaftar baru untuk Private Inner Circle Insider:\n\n*Nama:* ${nama}\n*Status:* ${status}\n*Company:* ${company}\n*Bidang:* ${bidang_usaha}\n*Domisili:* ${domisili}\n*WA:* ${whatsapp}\n*Email:* ${email}\n\n*Tujuan:* ${tujuan_meetup}\n\nMohon direview apakah cocok untuk diundang meet-up.`;
