...
serve(async (req)=>{
  ...
  try {
    ...
    const { subscriptionType, paymentMethod, tripayData, merchantRef } = await req.json();
    // Get or create pro subscription using UPSERT
    const { data: subscription, error: subError } = await supabaseClient.from('pro_subscriptions').upsert({
      user_id: user.id,
    ...
    const { error: transError } = await supabaseClient.from('payment_transactions').insert({
