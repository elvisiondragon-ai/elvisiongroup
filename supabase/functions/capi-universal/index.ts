...
const PIXEL_ID_VIP30JT = '1275074677473419';
...
Deno.serve(async (req) => {
  ...
  } else if (resolvedPixelId === PIXEL_ID_VIP30JT || resolvedPixelId === '1290886492233491') {
    FACEBOOK_ACCESS_TOKEN = FACEBOOK_ACCESS_TOKEN || Deno.env.get('VIP_30JT') || Deno.env.get('CAPI30JUTA');
  } else {
  ...
  try {
    ...
    // --- ADMIN EMAIL & TEST URL BLOCKER (Prevent Test Purchases from Hitting Ads Manager) ---
    const rawEmail = userData?.email || userData?.em;
    if (rawEmail) {
      const lowerEmail = rawEmail.trim().toLowerCase();
      const blockedEmails = ['elvisiondragon@gmail.com'];
      if (blockedEmails.includes(lowerEmail)) {
        console.log(`🚫 CAPI-UNIVERSAL: Blocked test event for admin email: ${lowerEmail}`);
        return new Response(JSON.stringify({ message: 'Event blocked by Admin Email filter', skipped: true }), {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    const sourceUrl = eventSourceUrl || req.headers.get('referer') || '';
    if (sourceUrl.includes('test=true') || sourceUrl.includes('test=1')) {
      console.log(`🚫 CAPI-UNIVERSAL: Blocked test event due to test flag in URL: ${sourceUrl}`);
      return new Response(JSON.stringify({ message: 'Event blocked by Test URL flag', skipped: true }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
