...
async function downloadToTmp(supabase, bucket, path) {
...
async function uploadFromTmp(supabase, bucket, destPath, filePath, contentType) {
  const file = await Deno.readFile(filePath);
  const { error } = await supabase.storage.from(bucket).upload(destPath, file, {
    upsert: true,
    contentType
  });
  if (error) throw error;
}
function deriveDestPath(srcPath) {
  const i = srcPath.lastIndexOf(".");
  if (i === -1) return `${srcPath}-compressed`;
  return `${srcPath.slice(0, i)}-compressed${srcPath.slice(i)}`;
}
function jpgQualityForHalf() {
  return 60;
}
function videoAudioBitrateForHalf() {
  return {
    v: "1500k",
    a: "128k"
  };
}
async function compressOne(supabase, destinationBucket, path) {
  ...
  return {
    path,
    output: destPathRelative
  };
}
async function listPrefixOnce(supabase, prefix) {
  const files = [];
  const folders = [];
...
async function listAllFilesRecursive(supabase, startPrefix = "") {
  ...
  try {
    if (req.method === "GET") {
      return new Response(JSON.stringify({
        status: "ok",
        message: `media-compress ready; source bucket: ${SOURCE_BUCKET}`
      }), {
        headers: {
    ...
    const { path, folder, recursive = true, destBucket } = body;
