...
const PRODUCT_TEMPLATES: Record<string, any> = {
  ...
  'ebook_feminine_ultimate': {
    ...
    instructions: [
      ...
      "Audio Love Magnet: dengarkan setiap malam sebelum tidur (wajib earphone).",
      "Q&A dengan admin tersedia — balas pesan WhatsApp ini kapan saja."
    ],
    lang: "id"
  },
  // --- DARK FEMININE EN ---
  ...
  'womenconsult': {
    subject: "Akses Download: Women AI Consultant — Materi & Bonus Anda",
    downloadLink: "https://drive.google.com/drive/folders/17jZmbTi9n0Ltu4-2P0YDuvdOmcFuWLJO?usp=sharing",
    color: "#9d174d",
    accentColor: "#ffffff",
    title: "Akses Women AI Consultant Terbuka!",
    description: "Terima kasih! Pembayaran Anda sukses. Folder ini berisi seluruh materi dan bonus Women AI Consultant Anda.",
    instructions: [
      "Klik tombol di bawah untuk mengunduh seluruh materi.",
      "Simpan ke perangkat Anda agar bisa dibaca kapan saja.",
...
function getProductKey(productName: string): string {
  ...
  if (lower.includes('ultimate')) return 'ebook_feminine_ultimate';
    ...
    if (reference && reference !== 'N/A') {
      try {
        ...
        if (SUPABASE_URL && SUPABASE_KEY) {
          console.log(`🔍 Auto-lookup reference: ${reference} from global_product`);
          const res = await fetch(`${SUPABASE_URL}/rest/v1/global_product?tripay_reference=eq.${reference}&select=*`, {
            headers: {
              'apikey': SUPABASE_KEY,
          ...
          if (products && products.length > 0) {
            const p = products[0];
            console.log(`✅ Reference found in global_product. Auto-filling details...`);
            if (!recipientEmail) recipientEmail = p.email;
            if (!userName) userName = p.name;
            if (productNameInput === 'Unknown Product' || !productNameInput) productNameInput = p.product_name;
        ...
        console.error(`❌ Failed to lookup reference from global_product:`, e);
      }
    }

    if (!recipientEmail) throw new Error('Email address is required. Provide userEmail or a valid reference.');
    if (!userName) userName = recipientEmail.split('@')[0];
