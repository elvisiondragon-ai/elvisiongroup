-- Final fix for all subscription type variations and complete admin dashboard setup

-- Update all invalid subscription types in pro_subscriptions first
UPDATE public.pro_subscriptions 
SET subscription_type = CASE 
  WHEN subscription_type IN ('1_day', '7_day') THEN 'trial'
  WHEN subscription_type IN ('30_day', '1_month') THEN 'monthly'
  WHEN subscription_type IN ('365_day', '1_year') THEN 'yearly'
  WHEN subscription_type NOT IN ('trial', 'monthly', 'yearly') THEN 'trial'
  ELSE subscription_type
END;

-- Update the constraint to include all variations we might encounter
...
CHECK (subscription_type IN ('trial', 'monthly', 'yearly'));

-- Update sync function with comprehensive normalization
...
BEGIN
  ...
  -- Comprehensive normalization of subscription type
  normalized_sub_type := CASE 
    WHEN NEW.subscription_type IN ('1_day', '7_day', 'trial') THEN 'trial'
    WHEN NEW.subscription_type IN ('30_day', '1_month', 'monthly') THEN 'monthly'
    WHEN NEW.subscription_type IN ('365_day', '1_year', 'yearly') THEN 'yearly'
    ELSE 'trial'  -- Default fallback
  END;
...
-- Update sync_all function with comprehensive normalization
...
BEGIN
  ...
  LOOP
    -- Comprehensive normalization of subscription type
    normalized_sub_type := CASE 
      WHEN rec.subscription_type IN ('1_day', '7_day', 'trial') THEN 'trial'
      WHEN rec.subscription_type IN ('30_day', '1_month', 'monthly') THEN 'monthly'
      WHEN rec.subscription_type IN ('365_day', '1_year', 'yearly') THEN 'yearly'
      ELSE 'trial'  -- Default fallback
    END;
...
-- Populate the table with cleaned data
