...
-- 1. First, fix the level calculation issue
...
-- 2. Create comprehensive days_remaining table
...
-- 3. Remove the duplicate trigger and keep only one comprehensive sync function
DROP TRIGGER IF EXISTS update_days_remaining_trigger ON public.pro_subscriptions;

-- 4. Create comprehensive sync function for days_remaining table
...
-- 5. Create the single comprehensive trigger
...
-- 6. Search function for admin dashboard
...
-- 7. Batch sync function for all records
...
BEGIN
  ...
  FOR rec IN 
    SELECT ps.*, decrypt_email(uci.email_encrypted) as email
  ...
  LOOP
    ...
    INSERT INTO public.days_remaining (
      ...
      rec.user_id,
      COALESCE(rec.email, rec.user_email, 'unknown@email.com'),
      rec.id,
...
-- 8. Populate the days_remaining table with existing data
