...
-- Insert the first super admin (you'll need to manually add this after migration)
-- This is commented out as it requires a specific user ID
...
-- 5. Add additional protection to payment transactions
-- Create more restrictive access logging
CREATE OR REPLACE FUNCTION public.secure_payment_access_check()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $function$
BEGIN
  -- Log every access to payment transactions
  PERFORM public.log_sensitive_action(
    'payment_transaction_accessed',
    'payment_transactions',
    NEW.id,
    jsonb_build_object(
      'user_id', auth.uid(),
      'transaction_owner', NEW.user_id,
      'amount', NEW.amount,
      'status', NEW.status,
...
-- Apply the trigger to monitor payment access
CREATE TRIGGER secure_payment_access_trigger
  AFTER SELECT ON public.payment_transactions
  FOR EACH ROW
  EXECUTE FUNCTION public.secure_payment_access_check();

-- 6. Create function to safely grant admin roles (only callable by super admins)
...
-- 7. Add emergency admin role revocation function
