...
export default function ShopAuto() {
  ...
  const saveSettings = async (overrides: any = {}, silent = true) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('profiles')
        .update({ shopauto_settings: settings })
        .eq('user_id', user.id);
  ...
  const handleTestChat = async () => {
    ...
    try {
      ...
      const currentEngine = aiProviderType === "system" ? "openai" : aiEngine;
      let aiResponse = "";
  ...
  if (!user) return <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white"><Button onClick={() => navigate('/auth')}>Login</Button></div>;
                    ...
                    <div className="space-y-4">
                      ...
                      {isWaConnected ? (
                        ...
                        <div className="flex flex-col items-center py-6 border-2 border-dashed border-orange-500/30 rounded-2xl bg-black/40 space-y-4">
                          ...
                          <Button variant="outline" size="xs" className="text-gray-500 text-[10px]" onClick={() => { setWaQrCode(null); setWaStatus("disconnected"); }}>Reset</Button>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center py-6 border-2 border-dashed border-orange-500/30 rounded-2xl bg-black/40 space-y-4">
                          ...
                          <Button variant="outline" size="xs" className="text-gray-500 text-[10px]" onClick={() => setWaStatus("connecting")}>Retry</Button>
                        </div>
                      )}
                    </div>
                  )}
