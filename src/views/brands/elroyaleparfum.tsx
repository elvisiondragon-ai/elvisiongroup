...
export default function ParfumPaymentPage() {
  ...
  const baseProductName = 'Parfum';
  const variants = {
    'eL Royale Phero': 600000,
    'eL Royale Egoiste': 600000,
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`payment-status-parfum-${paymentData.tripay_reference}`)
        ...
        (payload) => {
          if (payload.new?.status === 'PAID') {
            toast({
                ...
                duration: 0, 
                action: (() => {
                      const message = `Halo kak, saya sudah bayar untuk pesanan Parfum.\n\n` + 
                                      `Detail Pembayaran:\n` + 
                                      `- Nama: ${userName}\n` + 
                                      `- Email: ${userEmail}\n` + 
                                      `- Telepon: ${phoneNumber}\n` + 
  ...
  }, [showPaymentInstructions, paymentData?.tripay_reference, router, toast, userName, userEmail, phoneNumber, userAddress, selectedProvince, productName, totalQuantity, totalAmount, selectedPaymentMethod]);
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-center">
                  <img src={qrisBcaImage.src} alt="QRIS BCA" className="w-64 h-64 border rounded-lg" />
                </div>
                {paymentData.paymentMethod === 'BCA_MANUAL' && paymentData.status === 'UNPAID' && (
                  <div className="my-12">
                    <a
                      href={`https://wa.me/62895325633487?text=${encodeURIComponent(`Halo kak, saya sudah melakukan transfer manual BCA untuk pesanan Parfum.<br/><br/>` + 
      ...
      <div className="p-6 pb-4">
        <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold font-exo bg-gradient-primary bg-clip-text text-transparent">
                    Checkout Parfum eL Royale
                </h1>
            </div>
            {user ? (
                <Button variant="outline" onClick={signOut}>Logout</Button>
            ) : (
                <Button variant="outline" onClick={() => router.push('/auth?redirect=/elroyaleparfum')}>Login</Button>
            )}
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-center my-4">
              <img src={parfumImage.src} alt="Parfum Product" className="w-48 h-48 object-contain" />
            </div>
