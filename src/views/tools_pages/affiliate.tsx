...
export default function AffiliatePage() {
  ...
  const requestWithdrawal = async (amount: number) => {
    ...
    try {
      ...
      // Refresh withdrawals
      const { data } = await (supabase as any).from('withdrawals').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
      setWithdrawals(data || []);
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally {
  ...
  if (!user) return <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center space-y-4"><Button onClick={() => router.push('/auth')}>Login</Button></div>;
        ...
        <div className="bg-gradient-to-br from-gray-900 to-black border border-blue-900/30 rounded-lg p-6 mb-8 shadow-lg">
          ...
          <div className="grid grid-cols-1 gap-4">
            <Button
              ...
              onClick={() => router.push('/leadmagnet')}
            >
