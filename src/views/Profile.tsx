...
export function Profile({ onNavigate }: ProfileProps) {
  const { user, userId, userProfile, loading, cleanupSupabase, mediaAudit, signOut } = useAuth();
  ...
  useEffect(() => {
    ...
    const androidFullscreen = window.matchMedia('(display-mode: fullscreen)').matches;
    const androidMinimalUI = window.matchMedia('(display-mode: minimal-ui)').matches;
    
    setIsIOS(ios);
    setIsIOSStandalone(iosStandalone);
    setIsPWA((ios && iosStandalone) || androidStandalone || androidFullscreen || androidMinimalUI);
  }, []);
