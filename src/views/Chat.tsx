...
export function Chat({ onNavigate }: ChatProps) {
  ...
  const [chatProBadgeCache, setChatProBadgeCache] = useState(() => {
    if (typeof window === 'undefined') return { is_pro: false, is_admin: false, subscription_type: null, streak_days: 0, user_name: '', user_level: 1, avatar_url: '', timestamp: 0 };
  ...
  const loadMessages = useCallback(async (showRefreshState = false) => {
    ...
    try {
      ...
      // Get gold reports
      const { data: goldReports } = await (supabase as any)
        .from('gold_reports')
        .select('message_id');
      ...
      const userIds = Array.from(new Set(chatMessages.map(msg => msg.user_id)));
      ...
      if (uncachedUserIds.length > 0) {
        ...
        const { data: newSubscriptions } = await (supabase as any)
