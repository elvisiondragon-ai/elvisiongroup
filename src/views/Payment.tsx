...
export function Payment({ onNavigate }: PaymentProps) {
      ...
      .on(
        ...
        (payload) => {
          ...
          if ((payload.new as any)?.status === 'active') {
            ...
            if ((window as any).fbq && paymentData) {
            ...
            const timer = setInterval(() => {
              ...
              if (countdown <= 0) {
                ...
                onNavigate ? onNavigate("profile") : (window.location.href = "/");
              }
            }, 1000);
          } else {
            console.log('📝 Payment status update:', (payload.new as any)?.status);
          }
        }
      )
      .subscribe();
  ...
  useEffect(() => {
    if (user?.id && userProfile) {
      ...
      if ((userProfile as any).phone_number && !phoneNumber) {
        setPhoneNumber((userProfile as any).phone_number);
      }
    }
        ...
        <div className="fixed bottom-20 left-6 right-6 z-50">
          <Card className="p-4 bg-card/80 backdrop-blur-sm border border-border">
            <div className="space-y-3">
              ...
              <Button onClick={() => onNavigate ? onNavigate("profile") : (window.location.href = "/")} variant="outline" className="w-full">
                Kembali ke Profil
              </Button>
            </div>
          </Card>
      ...
      <div className="p-6 pb-4">
        <div className="flex items-center gap-4 mb-6">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => onNavigate ? onNavigate("profile") : (window.location.href = "/")}
            className="text-muted-foreground hover:text-foreground"
          >
            <ArrowLeft className="w-5 h-5" />
          </Button>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              {!phoneNumber.trim() && !(userProfile as any)?.phone_number?.trim() && (
